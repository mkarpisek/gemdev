...
/**
 * Renames existing method category.
 */
public class RenameCategory extends SessionAction<String> {
	private final String className;
	private final boolean instanceSide;
	private final String oldCategoryName;
	private final String newCategoryName;

	public RenameCategory(final String className, final boolean instanceSide, final String oldCategoryName, final String newCategoryName) {
	...
	public String asRequestString() {
		final String script = newScriptWithArguments(getClass().getSimpleName(), className, (instanceSide ? "true" : "false"), //$NON-NLS-1$//$NON-NLS-2$
				oldCategoryName.replaceAll("'", "''"), newCategoryName.replaceAll("'", "''") //$NON-NLS-1$//$NON-NLS-2$ //$NON-NLS-3$ //$NON-NLS-4$
		);
